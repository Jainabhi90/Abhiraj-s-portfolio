        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {SKILL_CATEGORIES.map((category, index) => (
              ...
              >
                <h3 className="text-xl font-semibold text-foreground mb-6 border-b border-glass-border pb-4">{category.title}</h3>
                <div className="flex flex-wrap gap-3">
                  {category.skills.map((skill, i) => (
                    <motion.span
                      key={i}
                      whileHover={{ scale: 1.05, backgroundColor: "rgba(52, 80, 112, 0.12)", color: "#345070", borderColor: "rgba(52, 80, 112, 0.3)" }}
                      className="px-4 py-2 rounded-lg bg-[var(--surface)] border border-glass-border text-sm font-medium text-foreground/80 cursor-default transition-all duration-300"
                    >
                      {skill}
                    </motion.span>
                  ))}
